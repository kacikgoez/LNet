...
'''
EDIT:
1. Copy of inference.py. Instead of re-computing the embeddings, we load the pre-computed embeddings of the last session
using pickle.

2. We also apply PCA defined in siameseQuick.py
'''
...
def siamese_inference(model, pca, domain_map, logo_feat_list, file_name_list, shot_path: str, gt_bbox, t_s, grayscale=False):
    '''
    ...
    img = None
    # CHANGED!: Either path or directly iamge
    if isinstance(shot_path, str):
        ...
        if isinstance(shot_path, PIL.Image.Image):
            img = shot_path

    ## get predicted box --> crop from screenshot
    cropped = img.crop((gt_bbox[0], gt_bbox[1], gt_bbox[2], gt_bbox[3]))
    if pca is not None:
        img_feat = pca.transform([pred_siamese(cropped, model, imshow=False, title='Original rcnn box', grayscale=grayscale)])[0]
    else:
